...
cluster: sync-git stop ## Create the kind cluster
	...
	for i in $$(seq $${ARGOCD_WAVE}); do \
		...
		if [ $$i -ge 2 ]; then \
			$(SCRIPT_DIR)/load-images; \
			while ! timeout 10 $(MAKE) --no-print-directory wait-all; do \
				$(SCRIPT_DIR)/load-images; \
			done; \
		fi; \
	done; \
...
pods: ## Show all pods in short format
	...
	| sed 's/Pending/pe/g; s/ErrImagePull/ep/g; s/ImagePullBackOff/pb/g' \
	| sed 's/PodInitializing/pi/g; s/ContainerCreating/cc/g; s/Error/Er/g' \
